...
import { ArrowLeft, Car, Sparkles, User, Zap, Database, CheckCircle2, Loader2, AlertCircle } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col justify-between">
        ...
        <div className="max-w-5xl mx-auto px-6 py-3.5 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-xs tracking-tight">AutoValuate AI</span>
            </div>
          </div>

          <div className="flex items-center gap-2 text-xs">
            <User className="w-3.5 h-3.5 text-slate-500" />
            <span className="text-slate-600">Created by <strong className="text-slate-900 font-semibold">Raaghav Bisht</strong></span>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-6 py-8 flex-1 w-full">
        <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Fill in car parameters to calculate machine learning market estimation.
            </p>
          </div>
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            <Button variant="outline" size="sm" onClick={() => loadPreset("swift")} className="text-xs h-7 px-2.5 border-slate-200 bg-white hover:bg-slate-100">
              Swift
            </Button>
            <Button variant="outline" size="sm" onClick={() => loadPreset("creta")} className="text-xs h-7 px-2.5 border-slate-200 bg-white hover:bg-slate-100">
              Creta
            </Button>
            <Button variant="outline" size="sm" onClick={() => loadPreset("city")} className="text-xs h-7 px-2.5 border-slate-200 bg-white hover:bg-slate-100">
              City
            </Button>
            <Button variant="outline" size="sm" onClick={() => loadPreset("nexon")} className="text-xs h-7 px-2.5 border-slate-200 bg-white hover:bg-slate-100">
              Nexon
            </Button>
          </div>
        </div>
        ...
        {errorMsg && (
          <div className="mb-6 p-3.5 rounded-lg bg-red-50 border border-red-200 text-red-700 flex items-center gap-2 text-xs">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Left Main Form Card */}
          <Card className="lg:col-span-2 p-6 minimal-card bg-white space-y-6">
            <Form {...form}>
              <form onSubmit={form.handleSubmit(handlePredict)} className="space-y-6">

                {/* Section 1: Brand & Model */}
                <div className="space-y-3">
                  <span className="text-xs font-bold text-slate-900 uppercase tracking-wider block border-b border-slate-100 pb-1.5">
                ...
                <div className="pt-2">
                  <Button
                    type="submit"
                    disabled={isLoading}
                    className="w-full h-11 text-xs font-semibold bg-slate-900 hover:bg-slate-800 text-white rounded-lg shadow-sm"
                  >
                    {isLoading ? (
                      <>
          ...
          <div className="lg:col-span-1 space-y-4">
            <Card className="p-5 minimal-card bg-white space-y-3">
              ...
              <div className="space-y-2 text-xs text-slate-600 leading-relaxed">
                <div className="p-2.5 rounded-lg bg-slate-100 flex justify-between items-center">
                  <span className="text-slate-600">Model Accuracy:</span>
                  <span className="font-bold text-slate-900">93.97% R²</span>
                </div>
        ...
        <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
          <DialogContent className="sm:max-w-[420px] bg-white text-slate-900 border-slate-200 rounded-xl">
            <DialogHeader>
            ...
            {result && (
              <div className="py-2 space-y-4">
                <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 text-center">
                  ...
                  <div className="text-3xl font-extrabold text-slate-900 py-1">
                ...
                <div className="p-3 rounded-lg bg-white border border-slate-200 text-xs space-y-1.5">
                  <div className="flex justify-between text-slate-500">
                    ...
                    <span className="font-semibold text-slate-900">R² Score: {result.r2_score}</span>
                  </div>
                  <div className="flex justify-between items-center font-semibold text-slate-800">
                    <span className="text-slate-700">{result.price_range.formatted_min}</span>
                    <span className="text-slate-400">—</span>
                    <span className="text-slate-900">{result.price_range.formatted_max}</span>
                  </div>
                </div>

                <Button onClick={() => setIsModalOpen(false)} className="w-full bg-slate-900 hover:bg-slate-800 text-white rounded-lg text-xs">
      ...
      {/* Footer */}
      <footer className="w-full border-t border-slate-200 py-3.5 bg-white text-center text-xs text-slate-500">
